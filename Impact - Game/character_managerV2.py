...
try:
    ...
    print(
        f"Warning: CharacterManager could not load tabulate. {e}. Perhaps you have not installed it?")
    quit()
...
try:
    ...
    print(
        f"Warning: CharacterManager could not load questionary. {e}. Please run 'pip install questionary'.")
    quit()
...
try:
    ...
    print(
        f"Warning: CharacterManager could not load time. {e}. Perhaps you have not installed it?")
    quit()
...
try:
    ...
    print(
        f"Warning: CharacterManager could not load random. {e}. Perhaps you have not installed it?")
    quit()
...
try:
    ...
    print(
        f"Warning: CharacterManager could not load character_attacks_database.py. {e}. Perhaps you have not installed it?")
    quit()
    ...
    def take_damage(
        ...
        if self.temporary_max_health > 0:
            if amount_of_damage <= self.temporary_max_health:
                ...
                # Damage is larger than temporary health, take away all temp
                # and subtract that from damage
        ...
        if self.health <= 0:
            self.health = 0
            self.alive = False
            print(f"{self.name} was defeated") if damaged_by == "Unknown" else print(
                f"{self.name} was defeated by {damaged_by}.")
            time.sleep(1)
        else:
            print(f"{self.name} took damage. Current health {self.health}") if damaged_by == "Unknown" else print(
                f"{self.name} took damage from {damaged_by}. Current health: {self.health}")
            time.sleep(1)
    ...
    def inflict_damage(self, target: CharacterManager, used_power: str):
        ...
        if used_power in power_data_list:
            ...
            attack_damage = random.randint(
                max(0, base_strength - 5), base_strength + 5)
            print(
                f"{self.name} {attack_info['power']} | Damage dealt: {attack_damage}")
            time.sleep(1.5)
            target.take_damage(attack_damage, damaged_by=self.name)
        else:
            print(
                f"Warning: {
                    self.name} does not know how to use {used_power}")
...
class Enemy(CharacterManager):
    ...
    def inflict_damage(self, target: CharacterManager, used_power: str):
        ...
        if used_power in power_data_list:
            ...
            base_strength = attack_info["strength"]
            attack_damage = random.randint(
                max(0, base_strength - 5), base_strength + 2)
            print(
                f"{self.name} {attack_info["power"]} | Damage dealt: {attack_damage}")
            time.sleep(1.5)
            target.take_damage(attack_damage, damaged_by=self.name)
    ...
    def gameloopCharacterBattle(self):
        ...
        while self.p1.alive and self.e1.alive:
            ...
            if enemy_choices:
                ...
                print(
                    f"Warning: CharacterBattle could not fetch moves for {
                        self.e1.name}")
            time.sleep(1)
