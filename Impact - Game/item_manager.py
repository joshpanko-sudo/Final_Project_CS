...
try:
    ...
    print(
        f"Warning: ItemManager could not load tabulate. {e}. Perhaps you have not installed it?")
    quit()
...
try:
    ...
    print(
        f"Warning: ItemManager could not load questionary. {e}. Perhaps you have not installed it?")
    quit()
...
class ItemManager():
    ...
    def add_item(self, item_name: str, item_amount: int, max_stack: int):
        ...
        while item_amount > 0:
            if len(self.items) >= self.item_capacity:
                # Inventory is full
                print(
                    f"{self.inventory_name} is full! Could not add remaining {item_amount}x {item_name}.")
                break
    ...
    def gameloopItemManager(self):
        command = questionary.select(f"{self.inventory_name} menu:", choices=[
                                     f"View {self.inventory_name}", "Find item", "Remove item", "Craft item"]).ask()
            ...
            match command:
                ...
                case "Remove item":
                    ...
                    confirm = questionary.select(
                        f"Remove {amount_to_remove} of {item_to_remove}?", choices=[
                            "Yes", "No"]).ask()
                    match confirm:
                        case "Yes":
                        ...
                        case _:
                            print(
                                "Warning: ItemManager cannot remove chosen item from inventory")
                            quit()
                case "Craft item":
