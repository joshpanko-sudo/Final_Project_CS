...
try:
    ...
    print(
        f"Warning: ItemManager could not load tabulate. {e}. "
...
try:
    ...
    print(
        f"Warning: ItemManager could not load questionary. {e}. "
        "Perhaps you have not installed it?")
    quit()
...
class CraftManager():
    ...
    def ShowCraftingRecipe(self):  # 3. Restored single definition with self
        """
        Print out the recpie
        """
        # Loop through every recipe item and its ingredients in 
        # the dictionary
        for recipe_name, ingredients in self.recipes.items():
            ...
            components = [
                f"{item}, {count}" for item,
                count in ingredients.items()]
...
class ItemManager():
    ...
    def add_item(self, item_name: str, item_amount: int, max_stack: int):
        '''
        Adds an item to the inventory. User can specify name, amount to 
        add, and a stack size for them item to stack.
        '''
        ...
        while item_amount > 0:
            if len(self.items) >= self.item_capacity:
                # Inventory is full
                print(
                    f"{self.inventory_name} is full! Could not add "
                    f"remaining {item_amount}x {item_name}.")
                break
            ...
            print(
                f"Items: {item_amount}x {item_name} has been added to {
                    self.inventory_name}")  # Summary of items added
    ...
    def remove_item(self, item_name: str, amount_to_remove):
        ...
        for i in range(
                len(self.items) - 1, -1, -1):  # Start -1, stop -1, step -1
            item = self.items[i]
        ...
        print(
            f"Removed {
                ...
                item_amount}x {item_name} from {
                self.inventory_name}")
        if item_amount > 0:
            print(
                f"Could not remove {item_amount}x {item_name} from {
                    self.inventory_name}.")
    ...
    def see_item(self, item_name: str):
        ...
        for item in self.items:
            if item.get("name") == item_name:
                print(
                    f'The item "{item_name}" is in the {
                        self.inventory_name}!')
                return True
            else:
                print(
                    f'The item "{item_name}" is not in the {
                        self.inventory_name}!')
                return False
    ...
    def gameloopItemManager(self):
        ...
        """
        command = questionary.select(f"{self.inventory_name} menu:", choices=[
                                     f"View {self.inventory_name}", 
        ...
        if command == f"View {self.inventory_name}":
            ...
            match command:
                case "Find item":
                    self.see_item(questionary.text("Item to "
                                                   "find: ").ask())
                case "Remove item":
                    item_to_remove = questionary.text("Item to "
                                                      "remove: ").ask()
                    amount_to_remove = questionary.text(
                        "Amount to remove: ").ask()
                    confirm = questionary.select(
                        f"Remove {amount_to_remove} of "
                        f"{item_to_remove}?", choices=[
                            "Yes", "No"]).ask()
                    match confirm:
                        case "Yes":
                ...
                case "Craft item":
                    ...
                    confirm = questionary.select(
                        f"Remove {amount_to_remove} of {item_to_remove}?", 
                        choices=[
                            "Yes", "No"]).ask()
                case _:
